...
Socket::Socket()
    : _max_fd(0), _current_sockets(), _current_clients(), _ready_sockets(), _ready_clients(), _fd_to_port(), _sockets(),
      _clients()
{
...
Socket::Socket(Socket const &copy)
    : _max_fd(copy._max_fd), _current_sockets(copy._current_sockets), _current_clients(copy._current_clients),
      _ready_sockets(copy._ready_sockets), _ready_clients(copy._ready_clients), _fd_to_port(copy._fd_to_port),
      _sockets(copy._sockets), _clients(copy._clients)
{
}
