            ...
            if (code == 0)
            {
                ...
                if (!isspace(file[i]))
                    throw std::invalid_argument(
                        "invalid value \"" + file.substr(tmp, skip_word_exception(file, i) - tmp) + "\"" + " in ...");
                braket_check(file, i);
                if (error < 300 || error > 599)
                    throw std::invalid_argument("value \"" + file.substr(tmp, skip_word_exception(file, i) - tmp) +
                                                "\" must be between 300 and 599 in ...");
            }
            else if (code == 1)
            {
                braket_check(file, i);
